...
export class CipherByTrialFinder {
    ...
    findAllResolvingSteps(board: Board, reversed: boolean=true, searchUntil: number): [boolean, Step[]] {
        let isSolving = false;
        let doLogging = true;
        let solutionSteps: Step[] = [];

        let currentTime = (new Date().getTime());
        if (currentTime > searchUntil) {
            if (doLogging && loggingActive) {
                console.log("### Maximal resolving time exceeeded.");
            }
            return [false, []];
        }
        ...
        if (fcCandidates[0].allowSet.length <= 3) {
            ...
            if (solutionSteps.length == 0) {
                ...
                for (let checkStep of possibleSolutionSteps) {
                    ...
                    try {
                        let foundSteps: Step[] = [];
                        [isSolving, foundSteps] = this.findAllResolvingSteps(testBoard, true, searchUntil);
